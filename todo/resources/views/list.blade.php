...
@section('contenu')
    ...
    @foreach($tasks as $task)
      ...
      <h3 class="titre_tache" style="background-color: #373737;">
              <a class="lien" id="{{$task->id}}" style="color: #cecece;">{{$task->name}}</a>
       ...
       <p>{{$task->descriptionTache}}
           ...
           @if($lists->where('task_id',$task->id) )
              <a style="color:red;"> (done : {{$lists->where('Accompli',1)->where('task_id',$task->id)->where('user_id',Auth::user()->id)->count()}}
         ...
         Créé le:  {{$task->created_at}}
          ...
          <a href="{{URL::to('/SeeSousTask/'.$task->id)}}">Voir vos sous-taches</a>
                ...
                <a type="button" style="border-radius:10px;width:100%;margin-top:-10px;" class="btn btn-primary btn-sm" href="{{URL::to('/NewTask/'.$task->id)}}" style="background-color: #373737;">Ajouter une sous-tache à {{$task->name}}</a>
<br>
                </div>
    @endforeach
    </div>
